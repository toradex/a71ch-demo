    ...
    U8 * pOutData, U16 * pOutDataLength)
{
  ...
  rv = RSA_private_decrypt(inDataLength, pInData, oaepMsg, lastGeneratedRsaKeyPair, RSA_NO_PADDING);
  if (rv < 0)
  {
...
int HOST_RSA_Encrypt(U8 * pInData, U32 inDataLength, U8 * pPubKey, U16 lengthPubKey, U8 * pOutData, U16 * pOutDataLength)
{
  ...
  rv = HOST_EME_OAEP_Encode(pInData, inDataLength, oaepMsg, &oaepMsgLen);
