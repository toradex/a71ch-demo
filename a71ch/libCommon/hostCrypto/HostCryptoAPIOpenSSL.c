    ...
    U8* outEncryptedData, U32* outEncryptedDataLen)
{
    ...
    if (pMechanismType->mechanism == HLSE_AES_ECB_ENCRYPT) {
        ...
        *outEncryptedDataLen = (U32)outEncryptedDataLenInt;
        ...
        if (inKeyLen >= 8) {
            nRet = DES_set_key((C_Block *)inKey, &ks1);
            if (nRet != 0) {
                return HOST_CRYPTO_ERROR;
            }
        }
        if (inKeyLen >= 16) {
            nRet = DES_set_key((C_Block *)(&inKey[8]), &ks2);
            if (nRet != 0) {
                return HOST_CRYPTO_ERROR;
            }
        }
        if (inKeyLen == 24) {
            nRet = DES_set_key((C_Block *)(&inKey[16]), &ks3);
            if (nRet != 0) {
                return HOST_CRYPTO_ERROR;
            }
        }
        ...
        for (i = 0; i < inDataLen; i += 8) {
            if (inKeyLen == 24) {
                DES_ecb3_encrypt((C_Block *)(inData + i), (C_Block *)(outEncryptedData + i), &ks1, &ks2, &ks3, DES_ENCRYPT);
            }
            else if (inKeyLen == 16) {
                DES_ecb2_encrypt((C_Block *)(inData + i), (C_Block *)(outEncryptedData + i), &ks1, &ks2, DES_ENCRYPT);
            }
            else {
                DES_ecb_encrypt((C_Block *)(inData + i), (C_Block *)(outEncryptedData + i), &ks1, DES_ENCRYPT);
            }
        }
    ...
    U8* outDecryptedData, U32* outDecryptedDataLen)
{
    ...
    if (pMechanismType->mechanism == HLSE_AES_ECB_DECRYPT) {
        ...
        *outDecryptedDataLen = outDecryptedDataLenInt;
        ...
        for (i = 0; i < inDataLen; i += 8) {
            if (inKeyLen == 24) {
                DES_ecb3_encrypt((C_Block *)(inData + i), (C_Block *)(outDecryptedData + i), &ks1, &ks2, &ks3, DES_DECRYPT);
            }
            else if (inKeyLen == 16) {
                DES_ecb2_encrypt((C_Block *)(inData + i), (C_Block *)(outDecryptedData + i), &ks1, &ks2, DES_DECRYPT);
            }
            else {
                DES_ecb_encrypt((C_Block *)(inData + i), (C_Block *)(outDecryptedData + i), &ks1, DES_DECRYPT);
            }
        }
    ...
    if (pMechanismType->mechanism == HLSE_AES_CMAC) {
        ...
        if (ret == HOST_CRYPTO_OK)
        {
            ...
            if (ret == HOST_CRYPTO_OK)
            {
                ...
                *outSignatureLen = outSignatureLenSizeT;
            }
        }
