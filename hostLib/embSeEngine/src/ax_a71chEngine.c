 ...
 * - If case the above does not apply; at compile time one can choose between two
 *   strategies:
 *   (1) return a fail
 *   (2) delegate the operation to the OpenSSL SW implementation
 */
...
static U16 getEcKeyReference(EC_KEY *eckey, axKeyIdentifier_t *validKeys, int nKeys,
...
static int EmbSe_Compute_Key(void *key, size_t outlen, const EC_POINT *pub_key, EC_KEY *ecdh,
  void *(*KDF)(const void *in, size_t inlen, void *out, size_t *outlen));
        ...
        const BIGNUM *inv, const BIGNUM *rp, EC_KEY *eckey)
{
    ...
    else if (sw == ERR_PATTERN_COMPARE_FAILED)
    {
        ...
        /* Attach OpenSSL's method to duplicate key */
        if (!ECDSA_set_method(dup_eckey, ECDSA_OpenSSL()))
        {
            EmbSe_Print(LOG_ERR_ON,"OpenSSL sign API set_method failure..\n");
            return NULL;
        }
        ...
        const ECDSA_SIG *sig, EC_KEY *eckey)
{
    ...
    else if ( (sw == ERR_NO_PRIVATE_KEY) || (sw == ERR_PATTERN_COMPARE_FAILED) )
    {
        ...
        EmbSe_PrintPayload(LOG_DBG_ON, refPub, pub_key_len, "PubKey (Verify)");
    ...
    else if (flagHandleKey == AX_ENGINE_INVOKE_OPENSSL_SW)
    {
        ...
        if (!ECDSA_set_method(dup_eckey, ECDSA_OpenSSL()))
        {
            EmbSe_Print(LOG_ERR_ON, "OpenSSL verify API ECDSA_set_method failure..\n");
            nRet = -1;
            goto clean_mem_up;
        }
 ...
 */
static int EmbSe_Compute_Key(void *sh_secret, size_t sec_len, const EC_POINT *pub_key, EC_KEY *ecdh,
  void *(*KDF)(const void *in, size_t inlen, void *out, size_t *outlen))
{
    U16 sw;
    ...
    else if (sw == ERR_PATTERN_COMPARE_FAILED)
    {
        ...
        // void *(*KDF)(const void *in, size_t inlen, void *out, size_t *outlen));
        ret = ECDH_compute_key(shSecBuf, shSecBufLen, pub_key, dup_ecdh, NULL);
...
err:
    ...
    if (pubKeyBuf) free(pubKeyBuf);
    if (shSecBuf) free(shSecBuf);
    return ret;
}
...
//  }
    {
        ...
        if (rjctServerAddress == NULL) {
            connectStatus = SM_RjctConnect("127.0.0.1:8050", &commState, Atr, &AtrLen);
        }
        else {
            char szConnect[256];
            ...
            if (sizeof(rjctServerAddress) > (connectLen-colonPortLen)) {
                ...
                connectStatus = SM_RjctConnect(szConnect, &commState, Atr, &AtrLen);
            }
        }
    }
#else
